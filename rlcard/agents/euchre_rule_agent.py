...
from rlcard.games.euchre_trick.utils import LEFT, NON_TRUMP_ORDER
...
class EuchreRuleAgent(object):
    ...
    def step(self, state):
        ...
        if len(hand) == 6:
            ...
            worst_card = [NON_TRUMP_ORDER.index(card[1]) for card in cards]
        ...
        if len(playable_trump) > 0:
            worst_card = [NON_TRUMP_ORDER.index(card[1]) for card in playable_trump]
        ...
        worst_card = [NON_TRUMP_ORDER.index(card[1]) for card in legal_actions]
        if len(worst_card) > 0:
